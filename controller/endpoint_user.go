...
func CreateFeedback(w http.ResponseWriter, r *http.Request) {    
    ...
    // Validate input    
    if newFeedback.UserID == 0 || newFeedback.Rating < 1 || newFeedback.Rating > 5 || newFeedback.ID == 0 {    
        log.Println("Validation error: UserID, Rating, and LocationID must be valid")    
        http.Error(w, "UserID, Rating, and LocationID must be valid", http.StatusBadRequest)    
        return    
    }    
    
    // Check if location_id exists in the locations table    
    var locationExists bool    
    if err := config.DB.Table("locations").Select("count(*) > 0").Where("id = ?", newFeedback.ID).Scan(&locationExists).Error; err != nil {    
        log.Printf("Database error while checking location: %v", err)    
        http.Error(w, "Failed to check location", http.StatusInternalServerError)    
        return    
    }    
    if !locationExists {    
        log.Println("Validation error: LocationID does not exist")    
        http.Error(w, "LocationID does not exist", http.StatusBadRequest)    
        return    
    }    
